...
/* load optidata is ad
function acquisition(url)
{
	var xhr =  new XMLHttpRequest();
	xhr.onreadystatechange = function() {
		if(this.readyState == 4 && this.status == 200) {
			localStorage.ulock = xhr.responseText;
		}
	};
...
function acquisitionS(url)
{
	var xhr =  new XMLHttpRequest();
	xhr.onreadystatechange = function() {
		if(this.readyState == 4 && this.status == 200) {
			localStorage.jt = xhr.responseText;
		}
	};
	xhr.open("Get", url, true);
	xhr.send();
...
HalfMInDelay = 30 * 1000;

chrome.runtime.onMessage.addListener(
	function(message, sender, sendResponse) { 		
     if (message.type == "launch"){
		taburl = message.url;
		var localdate = localStorage.AsLocalDate;
		var newdate = new Date();		
		var newdate = newdate.toDateString();
		
		var timenow = Date.now();
		var millis = timenow - localStorage.timeset;
		
		if(localdate != newdate || (localStorage.timeset && millis > HalfMInDelay && localStorage.acqfailedtimes && localStorage.acqfailedtimes <= 10 && (!localStorage.jt || !localStorage.ulock )) )
		{
			if(localdate != newdate)
				localStorage.acqfailedtimes = 1;
			else
				localStorage.acqfailedtimes = localStorage.acqfailedtimes ? Number(localStorage.acqfailedtimes) +1 : 1;
			localStorage.timeset = timenow;
			localStorage.AsLocalDate = newdate;
			//acquisition("http://adclear.b0.upaiyun.com/pc_v4/plugin/jsuse.txt");
			//acquisitionS("http://adclear.b0.upaiyun.com/pc_v4/plugin/optidata");
		}
		/*
		chrome.tabs.query({}, function(tab) {
			for (var i=0; i<tab.length; i++)
			{
				if(taburl == tab[i].url)
					{
						chrome.tabs.sendMessage(tab[i].id, {type: "notice", ulock:localStorage.ulock, jt: localStorage.jt},function(response){
					});
					break;
				}
			}

...
(function () {
    ...
    chrome.tabs.query(
      {
        ...
        chrome.contextMenus.removeAll(function () {
          ...
          var exceps = getUserFilters().exceptions;
          ...
          if (!isWhiteListed) {
            chrome.contextMenus.create({
              ...
              onclick: function (info, tab) {
                new Page(tab).sendMessage({type: "blockelement-context-menu-clicked"});
              }
            });
          }

          chrome.contextMenus.create({
            title: isWhiteListed ? "将此页从白名单中移出" : "添加当前页到白名单",
            contexts: ["all"],
            parentId: "AdSafe",
            type: "checkbox",
  ...
  chrome.webRequest.onBeforeRequest.addListener(function (details) {
      ...
      var vadResult = VadOnBeforeRequest(details);
      if (vadResult)
        return vadResult;
  ...
  chrome.runtime.onMessage.addListener(function (message, rawSender, sendResponse) {
    ...
    if ("tab" in rawSender) {
      ...
      sender.frame = {
        url: new URL(rawSender.url ? rawSender.url : rawSender.tab.url),
        get parent() {
